...
# Export der Fahrtrajektorie und Straßenanregung aus Blender
...
# Output-Ebene / späteres Bake-Ziel.
...
# Dieses Skript verwendet ausschließlich Fahrzeug_Pfadsteuerung

# als Referenz für die exportierten lokalen Größen.
...
# Wichtig:!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# - Nach der Änderung der Fahrspur oder der Fahrbahn MUSS die Animation neu gespielt werden,
#  und MUSS diese Skript erneut ausgeführt werden, um die Road Input CSV zu aktualisieren.
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
# - Dieses Skript verändert keine Federung.

# - Während des Exports wird die Szene frameweise ausgewertet.

# - Nach dem Export wird der ursprüngliche Frame wiederhergestellt.
...
RAD_RADIUS_M = 0.32
RAD_ENVELOPE_OFFSET_Y_LOCAL_M_LIST = [
    -0.25,
    -0.125,
    0.0,
    0.125,
...
def raycast_fahrbahn_world(
    ...
    fahrbahn_local_to_world_mat = fahrbahn_eval.matrix_world.copy()
    fahrbahn_world_to_local_mat = fahrbahn_eval.matrix_world.inverted()

    ray_start_fahrbahn_local_vec = (
        fahrbahn_world_to_local_mat @ ray_start_world_vec
    )

    ray_dir_fahrbahn_local_vec = (
        fahrbahn_world_to_local_mat.to_3x3() @ ray_dir_world_vec
    ).normalized()

    hit_bool, hit_fahrbahn_local_vec, hit_normal_fahrbahn_local_vec, hit_face_index = (
        fahrbahn_eval.ray_cast(
            ray_start_fahrbahn_local_vec,
            ray_dir_fahrbahn_local_vec,
            distance=ray_distance_m,
        )
    )

    if hit_bool:
        hit_world_vec = fahrbahn_local_to_world_mat @ hit_fahrbahn_local_vec

        hit_normal_world_vec = (
            fahrbahn_local_to_world_mat.to_3x3()
            @ hit_normal_fahrbahn_local_vec
        ).normalized()
    else:
        hit_world_vec = None
...
def read_rad_envelope_fahrbahn_current_frame(
    ...
    for rad_key, rad_object_name in RAD_OBJECT_NAMES.items():
        ...
        for rad_envelope_offset_y_local_m in RAD_ENVELOPE_OFFSET_Y_LOCAL_M_LIST:
            if abs(rad_envelope_offset_y_local_m) > RAD_RADIUS_M:
                raise Exception(
                    "rad_envelope_offset_y_local_m darf nicht groesser als RAD_RADIUS_M sein."
                )

            rad_envelope_vertical_radius_m = math.sqrt(
                RAD_RADIUS_M * RAD_RADIUS_M
...
def read_groessen_current_frame(frame):
...
def read_groessen_all_frames():
    ...
    for frame in range(frame_start, frame_end + 1):
        frame_data = read_groessen_current_frame(frame)
...
#
# Wirkung:
# - Beim Ausführen des Skripts wird nur die UI registriert.
# - Der Export startet NICHT automatisch.
# - Der Benutzer startet Tests oder Export über Buttons im Blender N-Panel.
#
# In Blender:
